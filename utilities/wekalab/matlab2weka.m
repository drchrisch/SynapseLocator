...
%
% cs09oct2017:  weka.core.Instance does not work! Changed into weka.core.DenseIntense (see
%               https://stackoverflow.com/questions/8582593/unable-to-instantiate-a-weka-class-in-matlab)
% 
% 
...
if iscell(data)         
    ...
    for i = 1:size(data,1)
        ...
        inst = weka.core.DenseInstance(numel(attributes));
...
% NUMERIC MATRIX
    ...
    % ADDING OBSERVATIONS
    arrayfun(@(x) instances.add(weka.core.DenseInstance(1,data(x,:))), 1:size(data,1)); % instance weight is 1
%     for i = 1:size(data,1)
%         % instances.add(weka.core.Instance(1,data(i,:))); % instance weight is 1
%         instances.add(weka.core.DenseInstance(1,data(i,:))); % instance weight is 1
%     end
